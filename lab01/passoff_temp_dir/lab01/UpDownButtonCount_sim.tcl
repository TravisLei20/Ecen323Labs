...
# set inc (btnu)=0 and dec (btnd)=0 for one clock cycle
add_force btnu 0
add_force btnd 0
run 10 ns
...
# rest for 5 clock cycles
run 50 ns

# set inc (btnu)=1 for five clock cycle
add_force btnu 1
run 50 ns

# set inc (btnu)=0 for one clock cycle
add_force btnu 0
run 10 ns

# set dec (btnd)=1 for two clock cycle
add_force btnd 1
run 20 ns

# set dec (btnd)=0 for one clock cycle
add_force btnd 0
run 10 ns

# set inc (btnu)=1 for two clock cycle
add_force btnu 1
run 20 ns
...
# Issue sw to value five for one clock cycle
add_force sw 0101
run 10 ns

# set plus (btnr)=1 for two clock cycle
add_force btnr 1
run 20 ns

# set plus (btnr)=0 for two clock cycle
add_force btnr 0
run 20 ns

# Issue sw to value two for one clock cycle
add_force sw 0010
run 10 ns

# set minus (btnl)=1 for four clock cycle
add_force btnl 1
run 40 ns

# set minus (btnl)=0 for one clock cycle
add_force btnl 0
run 10 ns
